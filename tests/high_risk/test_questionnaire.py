...
def test_questionnaire_push_waits_for_canonical_unionid_without_planning(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _event: (
            ...
            {"submission_id": "submission-current", "questionnaire_id": 7, "unionid": ""},
            [],
            "",
        ),
    )
    ...
    class MustNotPlan:
        def __init__(self) -> None:
            raise AssertionError("external effect planning must not start before identity is resolved")

    monkeypatch.setattr(event_consumers, "ExternalEffectService", MustNotPlan)
