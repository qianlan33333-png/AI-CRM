...
ALEMBIC_HEAD_REVISION = "0163_ai_audience_admin_member_read_indexes"
...
def test_empty_postgres_database_installs_and_reuses_alembic_head() -> None:
    with _isolated_database("install") as database_url:
        ...
        assert lane_policies == {
            ...
            "wecom_ai_assistant_bulk": (4, "blocked", True),
...
def test_alipay_order_created_index_upgrade_downgrade_and_reupgrade() -> None:
    index_name = "ix_alipay_pay_orders_created_id"
    with _isolated_database("alipay_order_created_index") as database_url:
        with psycopg.connect(database_url, autocommit=True) as connection:
            connection.execute(BASELINE_PATH.read_text(encoding="utf-8"))
        _upgrade_database_to(database_url, "0146_wechat_pay_event_lookup_index")
        ...
        _downgrade_database_to(database_url, "0146_wechat_pay_event_lookup_index")
        with psycopg.connect(database_url) as connection:
            downgraded = connection.execute(
                "SELECT to_regclass(%s)",
                (f"public.{index_name}",),
...
def test_order_source_sort_indexes_survive_additive_downgrade_and_reupgrade() -> None:
    index_names = {
        "idx_wechat_pay_orders_created",
        "idx_wechat_shop_orders_provider_created",
    }
    with _isolated_database("order_source_sort_indexes") as database_url:
        with psycopg.connect(database_url, autocommit=True) as connection:
            connection.execute(BASELINE_PATH.read_text(encoding="utf-8"))
        _upgrade_database_to(database_url, "0147_alipay_order_created_index")
        ...
        assert {row[0] for row in installed} == index_names
        assert all(row[1:] == (True, True) for row in installed)

        _downgrade_database_to(database_url, "0147_alipay_order_created_index")
        with psycopg.connect(database_url) as connection:
            downgraded = connection.execute(
                """
                SELECT indexname
        ...
        assert {row[0] for row in downgraded} == index_names
        ...
        assert {row[0] for row in reinstalled} == index_names
