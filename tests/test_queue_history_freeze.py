...
def test_internal_queue_metric_filters_only_join_events_when_required() -> None:
    def no_event_consumer_pairs(_value, _params) -> str:
        return ""

    empty_where, empty_params, empty_requires_event_join = queue_metric_filter_sql(
        {},
        event_consumer_pair_clause=no_event_consumer_pairs,
    )
    consumer_where, consumer_params, consumer_requires_event_join = queue_metric_filter_sql(
        {"consumer_names": ["projection"]},
        event_consumer_pair_clause=no_event_consumer_pairs,
    )
    event_where, event_params, event_requires_event_join = queue_metric_filter_sql(
        {"event_types": ["payment.succeeded"]},
        event_consumer_pair_clause=no_event_consumer_pairs,
    )

    assert (empty_where, empty_params, empty_requires_event_join) == ("", {}, False)
    assert consumer_where == "WHERE r.consumer_name = ANY(:consumer_names)"
    assert consumer_params == {"consumer_names": ["projection"]}
    assert consumer_requires_event_join is False
    assert event_where == "WHERE e.event_type = ANY(:event_types)"
    assert event_params == {"event_types": ["payment.succeeded"]}
    assert event_requires_event_join is True


def test_internal_queue_metrics_use_one_query_without_due_rows_and_two_with_due_rows(next_pg_schema) -> None:
    del next_pg_schema
    repository = SQLAlchemyInternalEventRepository()
    consumer_name = "queue-metrics-query-count"

    with request_query_telemetry_scope() as empty_telemetry:
        empty_metrics = repository.queue_metrics({"consumer_name": consumer_name})

    empty_snapshot = empty_telemetry.snapshot()
    assert empty_metrics["due_count"] == 0
    assert empty_metrics["due_count_by_event_type"] == {}
    assert empty_metrics["due_count_by_consumer"] == {}
    assert empty_snapshot.query_count == 1
    assert empty_snapshot.failed_query_count == 0

    event = repository.create_event(
        InternalEventCreateRequest(
            event_type="payment.succeeded",
            aggregate_type="order",
            aggregate_id="queue-metrics-query-count",
            idempotency_key="queue-metrics-query-count",
        )
    )
    repository.create_consumer_run(event=event, consumer_name=consumer_name)

    with request_query_telemetry_scope() as due_telemetry:
        due_metrics = repository.queue_metrics(
            {
                "event_type": "payment.succeeded",
    ...
    due_snapshot = due_telemetry.snapshot()
    assert due_metrics["due_count"] == 1
    assert due_metrics["due_count_by_event_type"] == {"payment.succeeded": 1}
    assert due_metrics["due_count_by_consumer"] == {consumer_name: 1}
    assert due_snapshot.query_count == 2
    assert due_snapshot.failed_query_count == 0
