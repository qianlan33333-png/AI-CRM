...
SCHEMA_CATALOG: list[dict[str, Any]] = [
    ...
    {
        "name": "audience_read.questionnaire_answers_v1",
        "description": "问卷首次完整提交筛选使用的只读答案投影，包含题目、选项快照和企微身份。",
        "columns": [
            {"name": "submission_id", "type": "bigint"},
            {"name": "questionnaire_id", "type": "bigint"},
            {"name": "questionnaire_title", "type": "text"},
            {"name": "question_id", "type": "bigint"},
            {"name": "question_type", "type": "text"},
            {"name": "question_title", "type": "text"},
            {"name": "selected_option_ids", "type": "jsonb"},
            {"name": "selected_option_texts", "type": "jsonb"},
            {"name": "external_userid", "type": "text"},
            {"name": "owner_userid", "type": "text"},
            {"name": "submitted_at", "type": "timestamptz"},
        ],
        "required_filters": ["questionnaire_id"],
        "recommended_filters": ["question_id", "selected_option_ids"],
        "example_sql": "SELECT DISTINCT external_userid FROM audience_read.questionnaire_answers_v1 WHERE questionnaire_id = :questionnaire_id",
    },
    {
    ...
    {
        "name": "audience_read.radar_clicks_v1",
        "description": "可归因雷达点击只读投影；模板按用户与雷达取首次点击时间。",
        "columns": [
            {"name": "click_id", "type": "bigint"},
            {"name": "radar_id", "type": "bigint"},
            {"name": "radar_code", "type": "text"},
            {"name": "radar_title", "type": "text"},
            {"name": "external_userid", "type": "text"},
            {"name": "owner_userid", "type": "text"},
            {"name": "clicked_at", "type": "timestamptz"},
        ],
        "required_filters": ["radar_id"],
        "recommended_filters": ["external_userid", "clicked_at"],
        "example_sql": "SELECT DISTINCT external_userid FROM audience_read.radar_clicks_v1 WHERE radar_id = :radar_id",
    },
    {
